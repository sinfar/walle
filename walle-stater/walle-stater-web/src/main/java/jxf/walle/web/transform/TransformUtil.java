...
public class TransformUtil {
    ...
    public static void transform(Object object) {
        ...
        collection.forEach(bean -> {
            ...
            Arrays.stream(declaredFields)
                    ...
                    .forEach(field -> {
                        // 递归转换
                        Object propertyValue = LambdaUtil.sure(() -> readMethodInvoke(bean, field.getName()));
    ...
    private void transformField(Object bean, Field field) {
        ...
        Object originalFieldValue = LambdaUtil.sure(() -> readMethodInvoke(bean, originalFieldName));
        ...
        // 转换结果写入当前字段
        LambdaUtil.sure(() -> writeMethodInvoke(bean, field.getName(), String.class, result));
    }
